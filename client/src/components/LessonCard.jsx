...
export default function LessonCard({ lesson }) {
  const { id, category, title, description, duration, completed, quizScore } = lesson
  return (
    <Link to={`/learn/${id}`} className={`lesson-card${completed ? ' lesson-card--done' : ''}`}>
      <div className="lesson-card__top">
        <span className="lesson-card__category">{category}</span>
        {completed && <span className="lesson-card__badge">✓ Done</span>}
      </div>
